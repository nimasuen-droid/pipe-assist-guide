...
export const Route = createFileRoute("/standards")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse standard pipe support types, functions, codes and editable tag prefixes.",
      },
    ],
  }),
...
function StandardsPage() {
  ...
  const [draft, setDraft] = useState<{ tagPrefix: string; notes: string }>({
    tagPrefix: "",
    notes: "",
  });
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Catalogue of standard support types per MSS SP-58/69 & PFI ES-26. Tag prefixes are
            editable and persist for future projects.
          </p>
        </div>
        <Button
          variant="outline"
          ...
          onClick={() => {
            resetStandards();
            toast.success("Standards restored to defaults");
          }}
        >
          ...
          <Card key={s.id} className="hover:border-primary/40 transition-colors">
            ...
            <CardContent className="text-sm space-y-2.5">
              ...
              <p className="text-xs text-muted-foreground">
                <b className="text-foreground">Typical use:</b> {s.typicalUse}
              </p>
          ...
          <div className="space-y-3">
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setDraft((d) => ({ ...d, tagPrefix: e.target.value.toUpperCase() }))
                }
                maxLength={6}
                className="font-mono"
              />
              <p className="text-[11px] text-muted-foreground mt-1">
                Up to 6 letters/digits. Used when this support type is tagged.
              </p>
            </div>
            <div>
      ...
      <p className="text-xs text-muted-foreground italic border-t border-border pt-3">
        Reference catalogue only. Final selection must follow project specifications and stress
        analysis.
      </p>
    </div>
  );
